...
/// These tokens extend the abstract Token type
/// defined in Compiler.Tokens
[<AutoOpen>]
module MinJ.Tokens
...
/// User facing errors generated by the MinJ.Tokens module.
/// These errors are generated as tokens in the token stream.
module Errors =

    /// Indicates that an int literal exceed allowed bounds.
    let NumberConstantOverflow = 
        sprintf <| "Number constants must be between %d and %d." 
                <| Int32.MinValue
                <| Int32.MaxValue
    
    /// Indicates that a char literal exceed allowed bounds.
    let OnlyASCIIChararacters = 
        "Only ASCII characters can be used as character constants."
    
    /// Indicates that a char literal is improperly formatted.
...
module InternalErrors =
    ...
    let InvalidTerminal =
        CompilerInternalException("Attempted to produced unknown terminal type. This is an error in the scanner DFA")
...
/// A mapping of terminals to string values
let terminalMap = 
...
/// Creates a terminal from a string and location.
/// The string is looked up in the terminal map and
/// the terminal is produced with the correct TerminalType.
let createTerminal str location = 
    try Terminal(terminalMap.[str], location) :> Token
    with | :? KeyNotFoundException as e -> 
        raise InternalErrors.InvalidTerminal
